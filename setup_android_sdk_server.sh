...
install_android_sdk() {
    ...
    if [ -d "cmdline-tools" ]; then
        echo "Removing existing cmdline-tools directory..."
        rm -rf cmdline-tools || handle_error "Failed to remove existing cmdline-tools directory"
    fi
    ...
    # Extract the command line tools into cmdline-tools directory
    echo "Extracting command line tools..."
    mkdir -p cmdline-tools || handle_error "Failed to create cmdline-tools directory"
    unzip -q "$CMDLINE_TOOLS_ZIP" -d cmdline-tools || handle_error "Failed to extract command line tools"

    # Clean up the ZIP file
    echo "Removing the ZIP file..."
    rm -f "$CMDLINE_TOOLS_ZIP" || handle_error "Failed to remove $CMDLINE_TOOLS_ZIP"

    # Move the extracted cmdline-tools to cmdline-tools/latest
    if [ -d "cmdline-tools/cmdline-tools" ]; then
        echo "Moving extracted cmdline-tools to cmdline-tools/latest..."
        mv cmdline-tools/cmdline-tools cmdline-tools/latest || handle_error "Failed to move cmdline-tools to cmdline-tools/latest"
    else
